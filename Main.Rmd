...
```{r Expand variables, results='hide'}
diff <- multi_window_and_lagsteps(diff, 
                                  AGGREGATE_WINDOWS, 
                                  LAGSTEPS,
                                  AGGREGATE_FUNCTIONS)
print(paste('Original Samples: ', str(nrow(data)), ', number of features: ', str(ncol(data)),sep=''))
print(paste('New Samples: ', str(nrow(diff)), ', number of features: ', str(ncol(diff)), sep=''))
```
...
gbmGrid1 <- expand.grid(interaction.depth = c(1,5,9),
                       ...
                       shrinkage = 0.1,
                       n.minobsinnode=20)
gbmFit1 <- train(High_SANDP_DiffP1 ~ ., data = training, 
                 method = "gbm", 
...
gbmGrid2 <- expand.grid(k= (3:9))
gbmFit2 <- train(High_SANDP_DiffP1 ~ ., data = training, 
                 method = "knn", 
                 trControl = fitControl,
                 verbose = FALSE,
                 tuneGrid = gbmGrid2)
...
#neuraal network neighbours
gbmGrid3 <- expand.grid(size= (3:9))
gbmFit3 <- train(High_SANDP_DiffP1 ~ ., data = training, 
                 method = "nnet", 
                 trControl = fitControl,
                 verbose = FALSE,
                 tuneGrid = gbmGrid3)
