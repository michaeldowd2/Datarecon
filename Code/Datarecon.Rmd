...
1. Scrape prediction data (eg for stock prices, market Opening price may be required) ^1^
1. Loads most recent saved models ^1^
2. Load the time series data, including scraped datapoints
3. Run predictive models ^1^
4. At a point when all data is published for current date, scrape the rest of the data (eg at market close, clowing price, high, low and volume are available) ^1^
2. Train a new set of regression and ensemble models using the most up-to-date data for the selected date
5. Saves these models ^1^
