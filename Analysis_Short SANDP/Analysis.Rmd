...
TARGET_COLS <- c('Close')
LAGGED_FEATURE_COLS <- c('High, Low, Close) #, Volume')
AVAILABLE_FEATURE_COLS <- c('Open')
...
master_dataframe <- function(Files, SkipCols) {
...
data <- master_dataframe(DATA_FILES, SKIP_COLUMNS)
features <- data %>%
  diff_dataframe(-1) %>%
